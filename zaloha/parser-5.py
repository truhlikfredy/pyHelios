...
def draw_acc(val=0):
	draw_gauge(acc,153*2,153*0,153,40,170+(300*float(val)))
...
def draw_radar(val=0):
	draw_gauge(radar,192*1,153*2,192,50,270+(conv_nonlinear(((0,0,0),(0.1838,20,65),(0.4631,50,165),(0.7541,150,270),(0.833,200,300),(0.9329,300,330),(1,350,340))
,float(val))))
...
def draw_adi(roll=0,pitch=0,sidelip=0):
	...
	window.blit(adi_horizont,(x,y))
	window.blit(adi_instrument,(x,y))

#        adi_kri=pygame.transform.rotozoom(adi_kridla,-float(roll)*180,1)
#	w,h=adi_kri.get_size()
#	print w,h
#	window.blit(adi_kri,(x+(size/2)-(w/2),y+(size/2)-(h/2)))
	window.blit(rot_center(adi_kridla,-float(roll)*180),(x,y))
...
while False:
	for l in range(5535):
		draw_temp(float(l)/5535,float(l)/1000)
		pygame.display.flip()
		time.sleep(0.01)
